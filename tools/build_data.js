...
// B9：开发期校验——事件 branches 与 relationDeltas 长度必须一致，否则运行时关系增量会错位（off-by-one）
let relWarn = 0;
for (const e of dedup) {
  for (const ev of (e.events || [])) {
    if (ev.branches && ev.relationDeltas && ev.branches.length !== ev.relationDeltas.length) {
      console.warn("[B9] 关系增量错位:", e.id, "/", ev.id || ev.title, "branches", ev.branches.length, "≠ relationDeltas", ev.relationDeltas.length);
      relWarn++;
    }
  }
}
if (relWarn) console.warn("⚠ 共", relWarn, "处 relationDeltas 与 branches 长度不一致（运行时静默错位，需修正数据）");
