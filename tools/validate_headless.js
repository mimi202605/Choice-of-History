...
;(async function(){
  ...
  try{
    ...
    // (h) AI 合并调用路径
    {
      window.COH_CONFIG={ apiBase:"http://mock", model:"m", builtinCred: COH_CIPHER.encrypt(JSON.stringify({api:"http://mock",key:"k",model:"m"})) };
      settings=Object.assign({},settings,{fastMode:false,offlineMode:false,builtin:true});
      const emp=EMPEROR_REGISTRY.find(e=>e.id==="qin_shihuang");
