...
def E(eid, title, kind, major, phase, res, chg, desc, flavor="", comment=""):
    return {
...
print("=== 随机时局池平衡校验（仅失败链四维负漂移报警；court 正偏/health 负偏为可接受）===")
for tag, bucket in POOLS.items():
    total += len(bucket)
    d = drift_of(bucket)
    n = len(bucket)
    # 报警规则：任一失败链维度净漂移 < -0.5*n（即单事件均值 < -0.5）视为危险负偏
    fail_neg = [k for k in FAIL_DIMS if d[k] < -0.5 * n]
    flag = "OK" if not fail_neg else "**失败链负偏:" + ",".join(fail_neg) + "**"
